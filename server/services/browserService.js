...
async function getActiveSession() {
    ...
    try {
        ...
        const activeSessionsResponse = await fetch("https://api.anchorbrowser.io/api/sessions/active", {
            method: "GET",
            headers: {
        ...
        if (activeSessionsResponse.ok) {
            ...
            if (lastSessionData && lastSessionData.id) {
                const isSessionActive = activeSessions.some(session =>
            ...
            if (activeSessions && activeSessions.length > 0) {
                ...
                console.log("Using most recent active session:", mostRecentSession.session_id);

                // Check if we have this session in Firestore
                const sessionQuery = await db.collection("browserSessions")
                ...
                if (!sessionQuery.empty) {
                    ...
                    console.log("Session found but no details in Firestore, creating new session");
                    return await createBrowserSession();
                }
            }
        } else {
            console.log("Failed to get active sessions, status code:", activeSessionsResponse.status);
        }
        ...
        if (lastSessionData && lastSessionData.id) {
            try {
                ...
                if (activeSessionsResponse.ok) {
                    ...
                    if (isSessionActive) {
                        console.log("Reusing session created by another request:", lastSessionData.id);
                        return lastSessionData.id;
                    }
                }
            } catch (error) {
...
export { getActiveSession, createBrowserSession, getLiveViewUrl, initializeFirebase };